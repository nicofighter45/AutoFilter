...
class Filter:
    ...
    def get_filtered_image(self, page_nb):
        ...
        if self.__filtered_images[page_nb] is None:
            ...
            self.__filtered_images[self.__page] = remove_lines(
                self.__images[self.__page].crop((0, 0, width, height // 2))
            )
    ...
    def __prepare_image(self):
        ...
        self.__images[self.__page] = Image.frombytes(
            "RGB", self.__size[self.__page], pix.samples
        )
    ...
    def __update_raw_text(self):
        ...
        text = pytesseract.image_to_string(
            self.__filtered_images[self.__page], lang="fra+eng"
        )
        self.__text.set_text(self.__page, text)

    def __get_cropped_raw_text(self, x, y, w, h):
        width, height = self.__size[self.__page][0], self.__size[self.__page][1]
        img = self.__filtered_images[self.__page].crop(
            (x * width, y * height * 2, w * width, h * height * 2)
        )
        return pytesseract.image_to_string(img, lang="fra+eng")

    def __handle_field(
        self, name, splitter, splitter_delta, *splitters, original_text=None
    ):
    ...
    def __handle_product_field(
        self, name, splitter, splitter_delta, *splitters, original_text=None
    ):
        self.__text.set_default_product(self.__page, name)
        text = self.__handle_field(
            name, splitter, splitter_delta, *splitters, original_text=original_text
        ).replace("À", "A")
        self.__text.set_product(self.__page, text)

    def __handle_revision_field(
        self, name, splitter, splitter_delta, *splitters, original_text=None
    ):
        text = self.__handle_field(
            name, splitter, splitter_delta, *splitters, original_text=original_text
        ...
        if len(text) == 1:
            text = "0" + text
        self.__text.set_product(
            self.__page, self.__text.get_product(self.__page) + "-" + text
        )

    def __handle_SN_field(
        self, name, splitter, splitter_delta, *splitters, original_text=None
    ):
    ...
    def __handle_designation_field(
        self, name, splitter, splitter_delta, *splitters, original_text=None
    ):
    ...
    def __analyse_page(self):
        if self.__has_title(
            ...
            self.__handle_designation_field(
                "Désignation", "\n", 0, "Nom", "Date", "Cde"
            )
