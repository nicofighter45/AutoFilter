...
class Layer:
    def __init__(self, app):
        ...
        self.name_input = InputText(
            "Nom du fichier", name_rect, (200, 200, 200), self.is_focused, ".pdf"
        )
        self.__product_input = InputText(
            "Produit", product_rect, (200, 200, 200), self.is_focused
        )
        self.__SN_input = InputText(
            "Numéro de Série", SN_rect, (200, 200, 200), self.is_focused
        )
        self.__designation_input = InputText(
            "Désignation", designation_rect, (200, 200, 200), self.is_focused
        )
        self.__inputs = [
            self.name_input,
            self.__product_input,
            self.__SN_input,
            self.__designation_input,
        ]
        ...
        self.__ok_button = Button(
            ...
            (0, 255, 0),
            ok_rect,
            function=lambda: self.change_file(
                lambda: self.__file.save(self.name_input.get_text())
            ),
            black_box=True,
            text_size=self.__text_size,
        )
        self.__return_button = Button(
            "RETURN",
            (245, 137, 54),
            return_rect,
            function=lambda: self.change_file(self.__file.return_to_previous),
            black_box=True,
            text_size=self.__text_size,
        )
        self.__no_button = Button(
            ...
            (255, 0, 0),
            no_rect,
            function=lambda: self.change_file(self.__file.delete),
            black_box=True,
            text_size=self.__text_size,
        )
    ...
    def __update_images(self):
        ...
        if text_filter is not None:
            ...
            if image is not None:
                self.__right_image = pg.image.fromstring(
        ...
        if text_filter is not None:
            ...
            if image is not None:
                self.__left_image = pg.image.fromstring(
                    image.tobytes(), image.size, image.mode
                )
    ...
    def __arrow_key(self, event):
        ...
        elif key == pg.K_RIGHT and self.__focused_field.cursor < len(
            self.__focused_field.get_text()
        ):
            if event.mod & pg.KMOD_CTRL:
            ...
            case pg.K_DELETE:
                if event.mod & pg.KMOD_CTRL:
                    self.__focused_field.delete_forward(
                        len(self.__focused_field.get_text())
                        - self.__focused_field.cursor
                    )
                    self.text_update()
                else:
                    self.__focused_field.delete_forward()
    ...
    def update(self):
        ...
        text_surface = font.render(
            f"{self.__page+1}/{self.__file.get_pages_number()}", True, (255, 255, 255)
        )
        self.__screen.blit(
            text_surface, text_surface.get_rect(midleft=(self.__page_text_rect))
        )
        text_surface = font.render(
            f"Fichier {self.__file.get_current_file_number()+1}/{self.__file.get_files_number()}",
            True,
            (255, 255, 255),
        )
        self.__screen.blit(
            text_surface, text_surface.get_rect(midleft=(self.__file_text_rect))
        )
        # Draw left image
        if self.__left_image:
            img = pg.transform.scale(
                self.__left_image,
                (self.__left_image_rect.width, self.__left_image_rect.height),
            )
        ...
        if self.__right_image:
            img = pg.transform.scale(
                self.__right_image,
                (self.__right_image_rect.width, self.__right_image_rect.height),
            )
